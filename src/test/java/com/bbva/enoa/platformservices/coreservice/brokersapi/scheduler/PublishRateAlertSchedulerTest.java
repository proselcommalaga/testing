...
class PublishRateAlertSchedulerTest
{
    @Mock
    private BrokerRepository brokerRepository;

    @Mock
    private PublishRateAlertScheduler.PublishRateChecker publishRateChecker;

    @InjectMocks
    private PublishRateAlertScheduler publishRateAlertScheduler;
    ...
    public void ok()
    {
        ...
        publishRateAlertScheduler.checkPublishRateInBrokers();

        verify(publishRateChecker).checkBroker(111);
        verify(publishRateChecker).checkBroker(222);
        verify(brokerRepository).findByStatus(BrokerStatus.RUNNING);
        verifyNoMoreInteractions(brokerRepository);
        verifyNoMoreInteractions(publishRateChecker);
    }
}