...
class ConsumerRateAlertSchedulerTest
{
    @Mock
    private BrokerRepository brokerRepository;

    @Mock
    private ConsumerRateAlertScheduler.ConsumerRateChecker consumerRateChecker;

    @InjectMocks
    private ConsumerRateAlertScheduler consumerRateAlertScheduler;
    ...
    public void ok()
    {
        ...
        consumerRateAlertScheduler.checkConsumerRateInBrokers();

        verify(consumerRateChecker).checkBroker(111);
        verify(consumerRateChecker).checkBroker(222);
        verify(brokerRepository).findByStatus(BrokerStatus.RUNNING);
        verifyNoMoreInteractions(brokerRepository);
        verifyNoMoreInteractions(consumerRateChecker);
    }
}