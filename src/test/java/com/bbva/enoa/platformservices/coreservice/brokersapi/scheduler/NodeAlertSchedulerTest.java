...
class NodeAlertSchedulerTest
{
    @Mock
    private BrokerRepository brokerRepository;

    @Mock
    private NodeAlertScheduler.NodeChecker nodeChecker;

    @InjectMocks
    private NodeAlertScheduler nodeAlertScheduler;
    ...
    public void ok()
    {
        ...
        nodeAlertScheduler.checkRunningBrokers();

        verify(nodeChecker).checkBroker(111);
        verify(nodeChecker).checkBroker(222);
        verify(brokerRepository).findByStatus(BrokerStatus.RUNNING);
        verifyNoMoreInteractions(brokerRepository);
        verifyNoMoreInteractions(nodeChecker);
    }
}